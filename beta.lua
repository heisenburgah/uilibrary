...
-- Updated with viewport-aware scaling and overflow handling
...
return function(shared, utility)
    ...
    local function getMaxUIHeight()
        local viewport = getViewportSize()
        -- Reserve 100px for taskbar/margins, use 90% of available height
        return math.floor(viewport.Y * 0.9) - 100
    end
    
    local function getMaxUIWidth()
        local viewport = getViewportSize()
        -- Use reasonable width, but cap it for very wide monitors
        return math.min(350, viewport.X * 0.25)
    end
    
    local function getSafePosition()
        local viewport = getViewportSize()
        -- Position UI safely within screen bounds
        return {
            x = math.min(50, viewport.X * 0.02), -- 2% from left or 50px, whichever is smaller
            y = math.min(80, viewport.Y * 0.05)  -- 5% from top or 80px, whichever is smaller
        }
    end
    ...
    function library:Window(windowProperties)
        ...
        -- Calculate safe UI dimensions
        local maxHeight = getMaxUIHeight()
        local maxWidth = getMaxUIWidth()
        ...
        function window:Movement(moveAction, moveDirection)
            if moveAction == "Movement" then
                ...
                --
                -- Handle scrolling when selection goes off-screen
                window:HandleScrolling()
        ...
        function window:HandleScrolling()
            if not window.content[window.currentindex] then return end
            
            local itemHeight = 17 -- Height per item
            local headerHeight = 23 -- Window header height
            local availableHeight = maxHeight - headerHeight
            local maxVisibleItems = math.floor(availableHeight / itemHeight)
            
            -- Calculate which items should be visible
            local selectedItemPosition = window.currentindex
            local startVisible = window.scrollOffset + 1
            local endVisible = window.scrollOffset + maxVisibleItems
            
            -- Scroll down if selection is below visible area
            if selectedItemPosition > endVisible then
                window.scrollOffset = selectedItemPosition - maxVisibleItems
            end
            
            -- Scroll up if selection is above visible area
            if selectedItemPosition < startVisible then
                window.scrollOffset = selectedItemPosition - 1
            end
            
            -- Clamp scroll offset
            window.scrollOffset = math.max(0, math.min(window.scrollOffset, #window.content - maxVisibleItems))
        end
        --
        ...
        -- // Main - Updated with viewport awareness
        local windowFrame = utility:Create("Square", {
            Visible = true,
            Filled = true,
            Thickness = 0,
            Color = shared.theme.inline,
            Size = UDim2.new(0, maxWidth, 0, 19),
            Position = UDim2.new(0, safePos.x, 0, safePos.y)
        }, "menu")
        --
        ...
        local scrollIndicator = utility:Create("Text", {
            Parent = windowFrame,
            Visible = false,
            Text = "↕",
            Outline = true,
        ...
        function window:Refresh()
            ...
            for index, page in pairs(window.pages) do
                local shouldShow = visibleCount >= window.scrollOffset and visibleCount < window.scrollOffset + maxVisibleItems
                page:Position(shouldShow and (headerHeight + ((visibleCount - window.scrollOffset) * itemHeight)) or -1000) -- Hide off-screen items
                page:SetVisible(shouldShow)
                ...
                if page.open then
                    for index, section in pairs(page.sections) do
                        local sectionShouldShow = visibleCount >= window.scrollOffset and visibleCount < window.scrollOffset + maxVisibleItems
                        section:Position(sectionShouldShow and (headerHeight + ((visibleCount - window.scrollOffset) * itemHeight)) or -1000)
                        section:SetVisible(sectionShouldShow)
                        contentCount = contentCount + 1
                        if sectionShouldShow then visibleCount = visibleCount + 1 end
                        --
                        for index, content in pairs(section.content) do
                            local contentShouldShow = visibleCount >= window.scrollOffset and visibleCount < window.scrollOffset + maxVisibleItems
                            content:Position(contentShouldShow and (headerHeight + ((visibleCount - window.scrollOffset) * itemHeight)) or -1000)
                            content:SetVisible(contentShouldShow)
                            if not content.noaction then
                                window.content[#window.content + 1] = content
                            end
                            contentCount = contentCount + 1
                            if contentShouldShow then visibleCount = visibleCount + 1 end
                        end
                    end
                end
            end
            
            -- Update window size - limit to available screen space
            local totalNeededHeight = math.min(headerHeight + (contentCount * itemHeight), maxHeight)
            utility:Update(windowFrame, "Size", UDim2.new(0, maxWidth, 0, totalNeededHeight))
            utility:Update(windowInline, "Size", UDim2.new(1, -2, 1, -4), windowFrame)
            
            -- Show scroll indicator if needed
            scrollIndicator.Visible = contentCount > maxVisibleItems
            if scrollIndicator.Visible then
                scrollIndicator.Text = string.format("↕ %d/%d", window.scrollOffset + maxVisibleItems, contentCount)
            end
        end
        --
        function window:Page(pageProperties)
            ...
            function page:Section(sectionProperties)
                ...
                function section:SetVisible(visible)
                    section.text.Visible = visible and section.text.Visible -- Respect original visibility state
                end
                --
                function section:Label(labelProperties)
                    ...
                    function label:SetVisible(visible)
                        label.text.Visible = visible and label.text.Visible
                    end
                    --
                ...
                --
                -- [Continue with other UI elements but add SetVisible method to each...]
                function section:Button(buttonProperties)
                    ...
                    function button:SetVisible(visible)
                        button.text.Visible = visible and button.text.Visible
                    end
                ...
                function section:Toggle(toggleProperties)
                    ...
                    function toggle:SetVisible(visible)
                        toggle.text.Visible = visible and toggle.text.Visible
                    end
                ...
                function section:Slider(sliderProperties)
                    ...
                    local sliderName = sliderProperties.name or sliderProperties.Name or "New Slider"
                    ...
                    function slider:SetVisible(visible)
                        slider.text.Visible = visible and slider.text.Visible
                    end
                ...
                function section:List(listProperties)
                    ...
                    function list:SetVisible(visible)
                        list.text.Visible = visible and list.text.Visible
                    end
                ...
                function section:MultiList(multiListProperties)
                    ...
                    function multiList:SetVisible(visible)
                        multiList.text.Visible = visible and multiList.text.Visible
                    end
                ...
                function section:PlayerList(playerListProperties)
                    ...
                    local playerListName = playerListProperties.name or playerListProperties.Name or "New PlayerList"
                    ...
                    function playerList:SetVisible(visible)
                        playerList.text.Visible = visible and playerList.text.Visible
                    end
                ...
                function section:Keybind(keybindProperties)
                    ...
                    function keybind:SetVisible(visible)
                        keybind.text.Visible = visible and keybind.text.Visible
                    end
                ...
                function section:ColorList(colorListProperties)
                    ...
                    local colorListName = colorListProperties.name or colorListProperties.Name or "New ColorList"
                    ...
                    function colorList:SetVisible(visible)
                        colorList.text.Visible = visible and colorList.text.Visible
                        colorList.color.Visible = visible and colorList.color.Visible
                    end
    ...
    function library:Notify(text, color) 
        if shared and shared.pointers["notifications"]:Get() then
            ...
            do -- Create Drawings with viewport-safe positioning
                notification.drawings.shadow_text = utility:Create("Text", {
                    ...
                    ZIndex = 99,
                    Visible = false,
                    Position = Vector2.new(viewport.X - 300, 50) -- Safe position for notifications
                }, "notification")
                ...
                notification.drawings.main_text = utility:Create("Text", {
                    ...
                    ZIndex = 100,
                    Visible = false,
                    Position = Vector2.new(viewport.X - 300, 50)
                }, "notification")
            end
